...
function getDefaultAttendancePeriod() {
  ...
  const week = Math.ceil(targetDate.getDate() / 7);
...
export default function AttendanceDashboard() {
  ...
  const filteredRecords = records
    .filter((r) =>
      selectedPlatoon ? r.platoon_slots?.platoon_name === selectedPlatoon : true
    )
    .filter((r) =>
      selectedSquad ? r.platoon_slots?.squad_name === selectedSquad : true
    );
  ...
  const platoonList = Array.from(
    new Set(records.map((r) => r.platoon_slots?.platoon_name).filter(Boolean))
  ).sort((a, b) => {
      ...
      <div className="relative z-10 flex w-full">
              ...
              <div key={type} className="mb-12">
                ...
                {Object.entries(platoons).map(([platoon, members]: any) => (
                  <div key={platoon} className="mb-8">
                    <h3 className="text-lg text-gray-300 mb-4">{platoon}</h3>
        ...
        <div className="w-[320px] border-l border-[#00ff66]/30 p-6 bg-black/50 backdrop-blur-xl">
          ...
          <div className="space-y-4 text-sm">
            <div>
              Total Records:
              <span className="text-[#00ff66] ml-2">{total}</span>
            </div>
