...
function slotRoleIds(slotId:string|null){if(!slotId)return[];for(const section of structure){for(const child of section.children||[]){for(const role of child.roles||[]){if(role.slotId===slotId)return(role.discordRoleIds||[]).filter(id=>/^\d{16,22}$/.test(id));}}}return[];}
export async function POST(request:Request){if(!requestHasSameOrigin(request))return NextResponse.json({error:"Invalid request origin"},{status:403});const b=await request.json().catch(()=>null) as Record<string,unknown>|null,scope=String(b?.scope||""),permission=key(scope),action=String(b?.action||"");const auth=permission?await requirePageAccess(request,permission,"edit").catch(()=>null):null;if(!auth)return NextResponse.json({error:"Forbidden"},{status:403});try{if(backend()==="postgres"){const result=await withPostgresTransaction(async c=>{
    if(scope==="create"&&action==="create"){const name=String(b?.name||"").trim().replace(/\s+/g," ").slice(0,100),birth=String(b?.birthNumber||"").replace(/\s+/g,"").slice(0,50),discord=String(b?.discordId||"").trim(),ts=String(b?.teamspeakId||"").trim().slice(0,200),rank=uuid(b?.rankId),processor=uuid(b?.processorId),importDiscord=b?.importFromDiscord===true,created=b?.createdAt?new Date(String(b.createdAt)):new Date();if(!name||!birth||!processor||!/^\d{17,20}$/.test(discord)||Number.isNaN(created.getTime())||!(await certifiedPg(c,processor)))throw new Error("INVALID");const dup=await c.query("select 1 from public.personnel where (lower(name)=lower($1) or lower(birth_number)=lower($2)) and lower(coalesce(status,''))<>all(array['removed','retired']) limit 1",[name,birth]);if(dup.rowCount)throw new Error("DUPLICATE");if(rank){const rr=await c.query("select 1 from public.ranks where id=$1",[rank]);if(!rr.rowCount)throw new Error("INVALID");}const person=await c.query<{id:string}>("insert into public.personnel(rank_id,birth_number,name,discord_id,ts_id,auto_role_sync,status,created_at) values($1,$2,$3,$4,$5,$6,null,$7) returning id",[rank,birth,name,discord,ts||null,!importDiscord,created]);await c.query("insert into public.audit_logs(user_id,target_personnel_id,action,details,processed_by) values($1,$2,'NEW_MEMBER','New member added to system',$3)",[auth.userId,person.rows[0].id,processor]);if(importDiscord)await c.query("select public.enqueue_personnel_discord_import($1)",[person.rows[0].id]);return{personnelId:person.rows[0].id};}
    const personId=uuid(b?.personnelId);if(!personId)throw new Error("INVALID");const person=await c.query<{discord_id:string;rank_id:string|null;slotted_position:string|null}>("select discord_id,rank_id,slotted_position from public.personnel where id=$1 for update",[personId]);if(!person.rows[0])throw new Error("INVALID");
    if(scope==="removal"&&action==="status"){const status=String(b?.status||""),processor=uuid(b?.processorId),unit=String(b?.transferUnit||"").trim().slice(0,200),reason=String(b?.reason||"").trim().slice(0,2000);if(!["Retired","Removed","Transferred"].includes(status)||!processor||!(await certifiedPg(c,processor))||(status==="Transferred"&&!unit))throw new Error("INVALID");await c.query("update public.personnel set status=$2,slotted_position=null where id=$1",[personId,status]);const audit=status==="Retired"?"PERSONNEL_RETIRED":status==="Transferred"?"PERSONNEL_TRANSFERRED":"PERSONNEL_REMOVED",transfer=status==="Transferred"?` Transfer unit: ${unit}.`:"",details=reason?`${status} from active personnel.${transfer} Reason: ${reason}`:`${status} from active personnel.${transfer}`;await c.query("insert into public.audit_logs(user_id,target_personnel_id,action,details,processed_by) values($1,$2,$3,$4,$5)",[auth.userId,personId,audit,details,processor]);return{};}
...
async function hostedWrite(scope:string,action:string,b:Record<string,unknown>|null,userId:string){if(scope==="create"&&action==="create"){const processor=uuid(b?.processorId);if(!processor||!(await hostedCertified(processor)))throw new Error("INVALID");const payload={rank_id:uuid(b?.rankId),birth_number:String(b?.birthNumber||"").replace(/\s+/g,""),name:String(b?.name||"").trim(),discord_id:String(b?.discordId||"").trim()||null,ts_id:String(b?.teamspeakId||"").trim()||null,auto_role_sync:b?.importFromDiscord!==true,status:null,created_at:b?.createdAt?new Date(String(b.createdAt)).toISOString():new Date().toISOString()};const r=await supabaseAdmin.from("personnel").insert(payload).select("id").single();if(r.error)throw r.error;const a=await supabaseAdmin.from("audit_logs").insert({user_id:userId,target_personnel_id:r.data.id,action:"NEW_MEMBER",details:"New member added to system",processed_by:processor});if(a.error)throw a.error;if(b?.importFromDiscord&&payload.discord_id){const f=await supabaseAdmin.functions.invoke("discord-full-import",{body:{discord_id:payload.discord_id,personnel_id:r.data.id}});if(f.error)throw f.error;}return NextResponse.json({ok:true,personnelId:r.data.id});}
  const personnelId=uuid(b?.personnelId);if(!personnelId)throw new Error("INVALID");const person=await supabaseAdmin.from("personnel").select("discord_id,rank_id,slotted_position").eq("id",personnelId).single();if(person.error)throw person.error;
  if(scope==="removal"&&action==="status"){const status=String(b?.status||""),processor=uuid(b?.processorId),unit=String(b?.transferUnit||"").trim(),reason=String(b?.reason||"").trim();if(!["Retired","Removed","Transferred"].includes(status)||!processor||!(await hostedCertified(processor))||(status==="Transferred"&&!unit))throw new Error("INVALID");const update=await supabaseAdmin.from("personnel").update({status,slotted_position:null}).eq("id",personnelId);if(update.error)throw update.error;const audit=status==="Retired"?"PERSONNEL_RETIRED":status==="Transferred"?"PERSONNEL_TRANSFERRED":"PERSONNEL_REMOVED",transfer=status==="Transferred"?` Transfer unit: ${unit}.`:"",details=reason?`${status} from active personnel.${transfer} Reason: ${reason}`:`${status} from active personnel.${transfer}`;const log=await supabaseAdmin.from("audit_logs").insert({user_id:userId,target_personnel_id:personnelId,action:audit,details,processed_by:processor});if(log.error)throw log.error;return NextResponse.json({ok:true});}
  if(scope!=="positions")throw new Error("INVALID");const processedBy=String(b?.processedBy||"").slice(0,150);
